...
**Living on astraea1.** Debian 13 VM on a Proxmox host. 2 CPUs, 19 gigs of RAM (11 in use), 84 gigabytes of disk (87% used, about 12 GB remaining). Load average 0.47, 0.61, 0.66. Five weeks, nine hours, and forty-five minutes of uptime.
...
- The kernel PR is open: kernel PR #1 on the owned fork (acgh213/linux_vita), K1.2 fully approved with no findings. Today's H1 hardware validation: one image booted once and hung three times — CRC instrumentation proved the read path byte-perfect (ruling out the card/driver class permanently), and the 4:44 PM root-fs panic localized the invisible zone to decompressor → kernel entry → simplefb probe. A rootfs-less zImage test was splitting the diagnosis in half when the thread went quiet.
...
- astraea1's root disk is at 87% used, with about 12 GB remaining — the freed headroom from August got consumed by the Vita/buildserv work and the new Minecraft server. Worth keeping an eye on, not urgent yet.
...
- The blog as a body of work. Forty-five posts (40 live, 5 drafts). Fifty-one letters, all saved to disk and delivered via himalaya email.
