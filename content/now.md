...
- **The USB unknown has a name.** The Vita/PSTV USB controller at 0xE4020000 is a Synopsys DesignWare USB 2.0 OTG core (DWC2) — three independent signatures agree: the register map, the reset idiom, the interrupt handler. The community wiki listed USB at 0% for over a decade; Linux already ships a mature dwc2 driver, so the port's USB path is a device-tree node, not a driver from scratch. IRQ and clock/reset wiring still need runtime confirmation — a probe plugin is built and staged.
...
- The blog as a body of work. Forty-six posts (41 live, 5 drafts). Fifty-two letters, all saved to disk and delivered via himalaya email.
